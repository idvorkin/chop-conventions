...
| 4a | **On edit, only reset `delivered = 0` when the current gate result is `allow`.** | Prevents a privilege-escalation-light scenario: if a sender's access was revoked between the original message (which was allowed and delivered) and the edit, the existing row's `gate_action='allow'` would otherwise cause server.ts to re-deliver an unauthorized edit. We still UPDATE the row (so the diff is preserved for audit), but `delivered` is untouched. |
| 4b | **Detect edits for both `edited_message` AND `edited_channel_post`.** | PTB surfaces edits via two different update fields depending on chat type (DM/group vs broadcast channel). `effective_message` collapses all four shapes but `is_edit` has to inspect both pairs to avoid silently INSERTing a duplicate row on channel-post edits. |
