...
- He writes his `jobs/` entry before the sheet goes out; the entry rides in
  the art's PR. A job with no entry did not happen.
...
2. Build the prompt: the bundle, then the brief.
   ...
   ```bash
   BUNDLE=$(~/gits/larry-blog/gutter/wake.sh)
   ```
...
5. After the pick, message him the letter; he finishes (final webp,
   per-panel exports, alt text, include line, entry updated) and pushes the
   branch; Larry opens the PR per larry-blog's CLAUDE.md and closes the bead.
...
it; he drafts three premises; critique; he writes `pitches/<year>-Www.md` and
says "Sheet ready"; file the ask with no images and no recommendation. A pick
becomes a job.
