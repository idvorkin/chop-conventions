...
# after a failure.
fast-test:
    @python3 -c "from pathlib import Path; import subprocess, sys; test_dirs = sorted({str(path.parent) for path in Path('skills').rglob('test_*.py')}); codes = [subprocess.run(['uv', 'run', '--quiet', '--no-project', '--with', 'pytest', '--with', 'typer', 'python', '-m', 'unittest', 'discover', '-s', test_dir, '-p', 'test_*.py']).returncode for test_dir in test_dirs]; sys.exit(0 if all(c == 0 for c in codes) else 1)"
