...
- **Always work in a worktree, never the primary checkout.** Multiple Claude sessions may share `~/gits/chop-conventions` at once — any branch checkout, rebase, or edit on the primary silently clobbers their state. Start every task with `git worktree add .worktrees/<slug> -b <branch> upstream/main` (or re-use an existing worktree) and do the work there. The primary checkout stays on `main` as a stable read surface for other sessions. This generalizes the global "Editing Skills: Symlink Trap" rule from skill files to all work in this repo — rebases, conflict resolution, feature branches, everything. Worktree dirs live under `.worktrees/` and are ignored via `.git/info/exclude`, so they never pollute branch history.
...
- **Adding personal-workflow entries to `.gitignore` mid-project is branch-dependent and bites on checkout.** If `.worktrees/` (or any similar dir) is ignored on branches that include the addition but not on older branches, checking out an older branch makes the dir show as untracked: `git status` noise, `git clean -fd` will wipe live worktrees, `git add -A` will stage them. Acceptable when the entry was present from the initial commit (every branch has it); otherwise prefer `.git/info/exclude`.
...
- **GitHub `mergeable: MERGEABLE` does not mean rebase-clean.** When a PR branch absorbed main via `git merge main` commits, GitHub's mergeability check reports MERGEABLE because the PR's recorded history already has the conflict resolution baked in — but a linear rebase onto the current base (dropping those merge commits) can still hit genuine conflicts. To deliver a single-commit PR: fetch the PR's original change commit, `git rebase upstream/main`, resolve, `--force-with-lease` to the fork. Don't trust `gh pr view`'s `mergeable` field as a proxy for "clean to rebase."
- **`git branch -d` refuses rebase-merged branches even when patch-equivalent to main.** After an upstream rebase-merge assigns a different SHA, the local branch's commit isn't literally an ancestor of `main`, so `-d` balks with "not fully merged." The `/up-to-date` skill's `absorbable_branches` (via `git cherry` patch-id) is the authoritative safety signal; when a branch is listed there, `-D` is safe regardless of merge style. Skill doc currently scopes `-D` fallback to `squash_merged_branches` only — broaden it or trust the absorbable-check result.
...
**Factual claims in generated output must be computed from the actual data, not hardcoded as constants.** LLM-written report code naturally reaches for narrative footnotes that _sound_ measured ("0 X observed", "no errors found") but are string literals that rot silently and can flip from true to false without anyone noticing. A TTL-bug footnote in `skills/cost-impact/_impl.py` unconditionally claimed `0 ephemeral_5m_input_tokens observed` — real data was 10.5M / $57.70. If a report asserts a number or category, derive it from the input; if you can't derive it, drop the assertion or phrase it as a question the reader must answer, not a fact. **Same rule for transform skills** (chroma-key, resize, transcript scrub, audio strip): compute a quality metric after the transform, print to stderr, exit nonzero on threshold violation under `--strict`. Visual inspection misses silent failures — the alpha channel, diff, or numeric check is the ground truth. See `skills/gen-image` for the `flood-fill + eval_alpha` pattern.
...
- `rmux_helper parent-pid-tree` — resolve the caller's owning tmux pane by walking `/proc/<pid>/stat` ppid chain and matching ancestors against `tmux list-panes -a -F '#{pane_pid}'`. **Use this for "which pane am I in" lookups from backgrounded subprocesses.** The obvious alternative — `tmux display-message -p '#{pane_id}'` — only does the right thing when it runs _inside_ the caller's tmux client context AND `TMUX_PANE` is honored (or `-t "$TMUX_PANE"` is passed explicitly). From a long-lived backgrounded process whose `TMUX_PANE` env var has gone stale (disowned, reparented, or forked across multiple tmux clients on the same box), unscoped `display-message` falls back to the session's most-recently-active pane and can silently target the wrong one. The parent-PID walk doesn't depend on env-var hygiene at all; it derives the answer from the kernel process tree, which survives reparenting and backgrounding intact. Observed 2026-04-14: `harden-telegram` watchdog hit the wrong pane for ~45 min under exactly this scenario (two concurrent Claude sessions, backgrounded watchdog) before the correct primitive was factored out.
...
- **Third-party skills and external binaries are tracked in [`dev-setup/external-tools.yaml`](dev-setup/external-tools.yaml), not in `skills/`** — `skills/` is for skills this repo authors and symlinks out. `/up-to-date` may run `dev-setup/tools/tool_doctor.py` and OFFER the install command for anything missing; it never runs one. See [`dev-setup/external-tools.md`](dev-setup/external-tools.md).
