...
- **`curl … | grep -q` under `pipefail` false-fails on success**: `grep -q` exits at its first match, curl then dies writing (exit 23), and pipefail fails the whole pipeline. Fetch to a temp file or variable first, then grep.
...
- **Merging a stacked PR lands on its BASE branch, not main.** GitHub retargets a stack to main only when the base branch is DELETED as its PR merges; merge the bottom PR without deleting, and the children's "Merged" badges are real while their commits strand off-main. After merging any stack, verify the content reached `git log origin/main`; recover by PR-ing the stranded branch tip into main.
